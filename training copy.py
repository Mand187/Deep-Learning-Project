...
def train_model(model_name, prediction_length, model_kwargs, train_loader, test_loader, loss_fn, gpu_id, optimizer_kwargs={}):
    ...
    trainScript = Trainer(model, train_loader, test_loader, model_name=model_name, model_path=model_file_path, device=device)  # Load the model from the specified path
    ...
    train_losses1, val_losses1, train_accs1, val_accs1, epoch_times1 = trainScript.train(
        ...
        criterion=loss_fn, 
        optimizer=torch.optim.AdamW(model.parameters(), lr=cfg.LEARNING_RATE, **optimizer_kwargs)
    )
